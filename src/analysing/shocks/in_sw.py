...
def in_solar_wind(spacecraft, time, speasy_variables, **kwargs):
...
def is_in_solar_wind(spacecraft, speasy_variables, start, end, **kwargs):
    ...
    if Vsw is None:
        Vsw = retrieve_data('V_GSE', 'OMNI', speasy_variables, start, end)
    ...
    if Pd is None:
        Pd = retrieve_data('P_dyn', 'OMNI', speasy_variables, start, end)
