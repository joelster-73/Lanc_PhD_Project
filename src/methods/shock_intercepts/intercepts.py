...
def find_shock_times(eventID, event, df_shocks, **kwargs):
    ...
    for sc in counts_dict:
        ...
        omni_pos, _ = retrieve_datum(position_var, 'OMNI', approx_time, add_omni_sc=False)
    ...
    pos, unc = retrieve_position_unc('OMNI', omni_time, omni_unc)
    ...
    for interceptor in sw_monitors:
        if interceptor in detectors:
            # Already have time
            continue
        elif interceptor in cluster_sc and (set(intercepted_sc) & set(cluster_sc)):
            # If found shock in one Cluster sc, don't need to check others
            continue
        elif interceptor in themis_sc and (set(intercepted_sc) & set(themis_sc)):
            # If found shock in one THEMIS sc, don't need to check others
            continue

        # Initial estimate of position
        intercept_pos, _ = retrieve_datum(position_var, interceptor, max_time, add_omni_sc=False)
        if intercept_pos is None or not in_solar_wind(interceptor, max_time):
            continue
        ...
        # If shock isn't found to intercept the spacecraft
        ...
        pos, unc = retrieve_position_unc(interceptor, intercept_time, intercept_unc)
        ...
        intercepted_sc.append(interceptor)

    if len(intercepted_sc)==0:
...
def find_propagation_time(shock_time, detector, interceptor, parameter, position=None, **kwargs):
    ...
    ###----------DETECTOR POSITION----------###
    if position is None:
        position, _ = retrieve_datum(position_var, detector, shock_time, add_omni_sc=False)
        if position is None:
    ...
    ###-------------------PARAMETER-------------------###
    data1 = retrieve_data(parameter, detector, start_up, end_up, downsample=True, resolution=sampling_interval, add_omni_sc=False)

    data2 = retrieve_data(parameter, interceptor, start_dw, end_dw, downsample=True, resolution=sampling_interval, add_omni_sc=False)
