...
def plot_quicklook(day, indices=('THL',), resolution='1min'):
    """
    A quicklook plot function of different indices on a certain day
    """
    nrows, ncols = get_grid_shape(len(indices))
...
def plot_mag_data_corr(station, sw, pc, param='H', coords='GSE', quantity='mag', ind='ER', show_sw=True, folder=None):
...
def plot_magnetometer_overview(df_station, df_omni, df_pc, start_time, end_time=None):
    ...
    for (ind, coords, quantity) in plot_combos:
        ...
        plot_mag_data_corr(station_sub, omni_sub, indices_sub, coords=coords, quantity=quantity, ind=ind, folder=folder)
...
def plot_compare_magnetometers(df_sw, df_pc, ind_var, dep='mag', restrict=True, shift_centre=True, **kwargs):
    ...
    file_name = f'Comparing_{ind_var}_{dep}_fit_{kwargs.get("fit_type","")}'
